...
function ArchetypeTag({ archetype }: { archetype: string }) {
  return (
    <span className="text-[9px] uppercase tracking-wide font-mono px-1.5 py-px rounded bg-(--color-surface-2) text-(--color-fg-subtle) shrink-0">
      {archetype}
    </span>
  );
}
  ...
  if (model.divergence === DIVERGENCE.humanFavored) {
    return (
      <InfoTip content={`Humans rate this above its benchmarks (Elo ${model.arena_elo}). Trust the crowd for chat.`}>
        <span className="inline-flex items-center gap-1 text-[9px] font-mono px-1.5 py-px rounded bg-(--color-success)/15 text-(--color-success) shrink-0">
  ...
  if (model.divergence === DIVERGENCE.benchFavored) {
    return (
      <InfoTip content={`Benchmarks rate this above human preference (Elo ${model.arena_elo}). Great scores, humans less sold.`}>
        <span className="inline-flex items-center gap-1 text-[9px] font-mono px-1.5 py-px rounded bg-(--color-warning)/15 text-(--color-warning) shrink-0">
...
function BenchmarkGrounding({ model }: { model: ModelStats }) {
  ...
  return (
    <InfoTip content={tip}>
      <span className="text-[10px] font-mono text-(--color-fg-subtle)/70">
